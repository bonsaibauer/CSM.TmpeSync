...
namespace CSM.TmpeSync.Util
{
    ...
    internal static partial class PendingMap
    {
        ...
        private static void EnsureWorker()
        {
            ...
            if (start)
                NetworkUtil.StartSimulationCoroutine(Worker());
        }
        ...
        private static void ProcessLaneAssignmentsInternal(Func<LaneAssignment, bool> predicate)
        {
            ...
            foreach (var guid in batch)
            {
                ...
                lock (Sync)
                {
                    ...
                    if (assignment.Attempts >= MaxLaneAssignmentAttempts || !NetworkUtil.SegmentExists(segmentId))
                    {
                        LaneAssignments.Remove(guid);
                        LaneAssignmentSet.Remove(guid);
                    }
        ...
        private static bool TryResolveLaneAssignment(LaneAssignment assignment)
        {
            ...
            if (LaneGuidRegistry.TryResolveLane(laneGuid, out var resolvedLaneId) && resolvedLaneId != 0 && NetworkUtil.LaneExists(resolvedLaneId))
            {
                assignment.LaneId = resolvedLaneId;
                if (segmentId == 0 || laneIndex < 0)
                {
                    if (!NetworkUtil.TryGetLaneLocation(resolvedLaneId, out segmentId, out laneIndex))
                        return false;
                }
            ...
            if (LaneMappingStore.TryResolveLaneGuid(laneGuid, out var entry))
            {
                ...
                if (localLaneId != 0 && NetworkUtil.LaneExists(localLaneId))
                {
            ...
            if (segmentId != 0 && laneIndex >= 0 && NetworkUtil.TryGetLaneId(segmentId, laneIndex, out var laneId))
            {
