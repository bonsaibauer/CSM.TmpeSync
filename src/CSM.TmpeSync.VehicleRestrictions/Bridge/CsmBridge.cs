...
namespace CSM.TmpeSync.VehicleRestrictions.Bridge
{
    public static class CsmBridge
    {
        ...
        public static void SendToClient(int clientId, CommandBase command)
        {
            if (command == null)
                return;
            ...
            if (!IsServerInstance())
            {
                ...
                Log.Debug(LogCategory.Network, "SendToClient ignored on non-server instance | type={0}", commandName);
                return;
            }
        ...
        private static bool TrySendToClientInternal(int clientId, CommandBase command, out string failureReason)
        {
            ...
            try
            {
                ...
                {
                    failureReason = "command_internal_null";
                    return false;
                }

                var manager = _multiplayerManagerInstanceProperty?.GetValue(null, null);
                var server = _multiplayerManagerCurrentServerProperty?.GetValue(manager, null);
                if (server == null)
                {
        ...
        public static int GetSenderId(CommandBase command) => command?.SenderId ?? -1;

        public static bool IsServerInstance() => Command.CurrentRole == MultiplayerRole.Server;

        public static string DescribeCurrentRole() => Command.CurrentRole.ToString();

        public static IDisposable StartIgnore()
        {
            try
            {
        ...
        public static void EnsureStubSimulationActive()
        {
        ...
        public static void ResetStubSimulationState()
        {
