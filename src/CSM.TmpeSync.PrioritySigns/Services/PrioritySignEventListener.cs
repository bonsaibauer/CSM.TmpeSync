    ...
    internal static class PrioritySignEventListener
    {
        ...
        internal static void Enable()
        {
            ...
            try
            {
                ...
                if (!patchedAny)
                {
                    Log.Warn(LogCategory.Network, LogRole.Host, "[PrioritySigns] Harmony listener disabled | reason=no_patch_targets.");
                    _harmony = null;
                    return;
                }
            ...
            {
                Log.Error(LogCategory.Network, LogRole.Host, "[PrioritySigns] Harmony listener enable failed | error={0}.", ex);
            }
        }
        ...
        internal static void Disable()
        {
            ...
            try
            {
                ...
                Log.Info(LogCategory.Network, LogRole.Host, "[PrioritySigns] Harmony listener disabled.");
            }
            catch (Exception ex)
            {
                Log.Warn(LogCategory.Network, LogRole.Host, "[PrioritySigns] Harmony listener disable failed | error={0}.", ex);
            }
            finally
            {
                _harmony = null;
        ...
        private static bool TryPatchSetPrioritySign()
        {
            ...
            Log.Info(
                LogCategory.Network,
                LogRole.Host,
                "[PrioritySigns] Harmony patched {0}.{1}({2}).",
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                else
                {
                    Log.Warn(
                        ...
                        LogRole.Host,
                        "[PrioritySigns] TryRead failed during node broadcast | nodeId={0} segmentId={1}.",
                        nodeId,
                        segId);
                }
        ...
        private static void SendLocalChange(ushort nodeId, ushort segmentId, PrioritySignType signType, string context)
        {
            if (CsmBridge.IsServerInstance())
            {
                Log.Info(
                    LogCategory.Synchronization,
                    LogRole.Host,
                    "[PrioritySigns] Host applied | nodeId={0} segmentId={1} sign={2} context={3}.",
                    nodeId,
                    segmentId,
                    signType,
                    context);
            ...
            Log.Info(
                LogCategory.Network,
                LogRole.Client,
                "[PrioritySigns] Client sent update request | nodeId={0} segmentId={1} sign={2} context={3}.",
                nodeId,
                segmentId,
                signType,
                context);
