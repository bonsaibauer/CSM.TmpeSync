...
namespace CSM.TmpeSync.ClearTraffic.Handlers
{
    public class ClearTrafficAppliedCommandHandler : CommandHandler<ClearTrafficAppliedCommand>
    {
        ...
        {
            Log.Info(LogCategory.Network, LogRole.Client, "[ClearTraffic] Applied command received | origin=remote.");
            ...
            NetworkUtil.RunOnSimulation(() =>
            {
                using (CsmBridge.StartIgnore())
                {
                    ...
                    {
                        Log.Info(LogCategory.Synchronization, LogRole.Client, "[ClearTraffic] Apply completed | result=success.");
                    }
                    else
                    {
                        Log.Error(LogCategory.Synchronization, LogRole.Client, "[ClearTraffic] Apply completed | result=failed.");
                    }
                }
            });
        }
