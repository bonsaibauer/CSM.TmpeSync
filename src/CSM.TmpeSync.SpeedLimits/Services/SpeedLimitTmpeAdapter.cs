            ...
            try
            {
                ...
                using (LocalIgnore.Scoped())
                {
                    ...
                    foreach (var item in request.Items)
                    {
                        ...
                        {
                            Log.Warn(LogCategory.Synchronization, LogRole.Host, "[SpeedLimits] Apply skipped | segmentId={0} ordinal={1} reason=invalid_ordinal.", segmentId, idx);
                        ...
                        if (!IsLaneConfigurable(laneInfo))
                        {
                            Log.Warn(LogCategory.Synchronization, LogRole.Host, "[SpeedLimits] Apply skipped | segmentId={0} ordinal={1} reason=not_vehicle_lane.", segmentId, idx);
                            okAll = false; continue;
                        }
                        if (!SignatureMatches(laneInfo, item.Signature))
                        {
                            Log.Warn(LogCategory.Synchronization, LogRole.Host, "[SpeedLimits] Apply skipped | segmentId={0} ordinal={1} reason=signature_mismatch.", segmentId, idx);
                            okAll = false; continue;
                        }

                        if (!NetworkUtil.TryGetLaneId(segmentId, idx, out var laneId))
                        {
                            Log.Warn(LogCategory.Synchronization, LogRole.Host, "[SpeedLimits] Apply skipped | segmentId={0} ordinal={1} reason=laneid_resolve_failed.", segmentId, idx);
                            okAll = false; continue;
                        }
                        ...
                        {
                            Log.Warn(LogCategory.Synchronization, LogRole.Host, "[SpeedLimits] Apply failed | segmentId={0} ordinal={1} reason=tmpe_apply_failed.", segmentId, idx);
                            okAll = false; continue;
                        }
                    }
