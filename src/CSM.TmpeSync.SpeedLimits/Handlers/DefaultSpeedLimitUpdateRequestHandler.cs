    ...
    public class DefaultSpeedLimitUpdateRequestHandler : CommandHandler<DefaultSpeedLimitUpdateRequest>
    {
        protected override void Handle(DefaultSpeedLimitUpdateRequest command)
        {
            ...
            Log.Info(
                LogCategory.Network,
                LogRole.Host,
                "[SpeedLimits] Default update request received | netInfo={0} custom={1} value={2:F3} senderId={3}.",
            ...
            {
                Log.Debug(LogCategory.Network, LogRole.Client, "[SpeedLimits] Default update request ignored | reason=not_server_instance.");
                return;
            }

            if (command == null || string.IsNullOrEmpty(command.NetInfoName))
            {
                Log.Warn(LogCategory.Network, LogRole.Host, "[SpeedLimits] Default update request rejected | reason=invalid_payload senderId={0}.", senderId);
                    ...
                    switch (error)
                    {
                        case SpeedLimitSynchronization.DefaultApplyError.NetInfoMissing:
                            Log.Warn(
                                LogCategory.Network,
                                LogRole.Host,
                                "[SpeedLimits] Default update request rejected | netInfo={0} reason=netinfo_missing senderId={1}.",
                                command.NetInfoName,
                                senderId);
                        ...
                        default:
                            Log.Error(
                                ...
                                LogRole.Host,
                                "[SpeedLimits] Default apply failed | netInfo={0} reason={1} senderId={2}.",
                                command.NetInfoName,
                                error,
                                senderId);
