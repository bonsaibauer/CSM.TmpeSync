...
function Update-ModMetadataFile {
    param(
        ...
        [string]$ExpectedCitiesSkylinesVersionRaw = "",
        [switch]$DryRun
    )
    ...
    $existingExpectedLine = & $extractExisting 'ExpectedCitiesSkylinesVersionMajorMinor' $currentText
    ...
    $existingExpectedRaw = & $extractExisting 'ExpectedCitiesSkylinesVersionRaw' $currentText
    if ([string]::IsNullOrWhiteSpace($ExpectedCitiesSkylinesVersionRaw)) {
        $ExpectedCitiesSkylinesVersionRaw = $existingExpectedRaw
    }

    $escapedVersion = & $escapeValue $existingVersion
    $escapedExpectedVersionLine = & $escapeValue $ExpectedCitiesSkylinesVersionLine
    $escapedExpectedVersionRaw = & $escapeValue $ExpectedCitiesSkylinesVersionRaw
    ...
    $lines = @(
        ...
        '        /// </summary>',
        "        internal const string ExpectedCitiesSkylinesVersionMajorMinor = ""$escapedExpectedVersionLine"";",
        '',
        '        /// <summary>',
        '        /// Full detected Cities: Skylines version string (for display/debug in checks).',
        '        /// </summary>',
        "        internal const string ExpectedCitiesSkylinesVersionRaw = ""$escapedExpectedVersionRaw"";",
        ''
    )
...
function Invoke-CsmTmpeSyncUpdate {
    ...
    $expectedCitiesSkylinesVersionRaw = ''
    if ($csVersionInfo -ne $null) {
        $expectedCitiesSkylinesVersionLine = [string]$csVersionInfo.VersionLine
        $expectedCitiesSkylinesVersionRaw = [string]$csVersionInfo.RawVersion
        Write-UpdateInfo "Detected local Cities: Skylines version | raw=$expectedCitiesSkylinesVersionRaw line=$expectedCitiesSkylinesVersionLine source=$($csVersionInfo.SourcePath)"
        Write-UpdateInfo "Cities: Skylines metadata update successful | line=$expectedCitiesSkylinesVersionLine raw=$expectedCitiesSkylinesVersionRaw"
    }
    else {
    ...
    Update-ModMetadataFile `
        ...
        -ExpectedCitiesSkylinesVersionRaw $expectedCitiesSkylinesVersionRaw `
        -DryRun:$false
    ...
    if ($change -match '^(?i)y(es)?$') {
        ...
        if (-not [string]::IsNullOrWhiteSpace($newVersion)) {
            ...
            if ($newContent -ne $content) {
                ...
                Write-UpdateInfo "NewVersion updated to: $newVersion"
            } else {
                Write-UpdateWarn "No match found for NewVersion. File unchanged."
            }
        } else {
            Write-UpdateInfo "Empty input. NewVersion remains unchanged."
        }
    } else {
        Write-UpdateInfo "No change requested for NewVersion."
    }
